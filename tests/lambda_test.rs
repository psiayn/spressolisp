...
fn test_tail_recursion() {
    ...
    eval_expr_in_env(
        "(define fact_tail (lambda (n acc) (if (== n 0) acc (fact_tail (- n 1) (* n acc)))))",
        &mut env,
    );
    eval_expr_in_env(
        "(define factorial_tail (lambda n (fact_tail n 1)))",
        &mut env,
    );
...
fn test_mutual_recursion() {
    ...
    eval_expr_in_env(
        "(define is_even (lambda n (if (== n 0) true (is_odd (- n 1)))))",
        &mut env,
    );
    eval_expr_in_env(
        "(define is_odd (lambda n (if (== n 0) false (is_even (- n 1)))))",
        &mut env,
    );
...
fn test_recursive_list_processing() {
    ...
    eval_expr_in_env(
        "(define sum_list (lambda lst (if (empty? lst) 0 (+ (nth lst 0) (sum_list (rest lst))))))",
        &mut env,
    );
